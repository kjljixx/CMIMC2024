...
class BlottoSwarmGame:
    ...
    def __init__(self) -> None:
        ...
        # self.board = [[0, 3, 6, 1, 1, 4, 2, 5, 2, 2, 0, 2, 1, 1, 1, 3, 3, 1, 2, 1, 1, 2, 2, 1, 1, 4, 1, 5, 2, 0], [0, 2, 3, 1, 1, 3, 2, 1, 2, 1, 1, 2, 5, 1, 0, 0, 3, 1, 2, 3, 2, 4, 3, 1, 3, 4, 2, 0, 3, 4]]
        # self.soldiers = [[], []]
        # for team in range(2):
        #     for i in range(self.board_sz):
        #         for j in range(self.board[team][i]):
        #             self.soldiers[team].append(i)
    ...
    def __str__(self):
        ...
        for i in range(self.board_sz):
            if(i % 3 == 2):
                s += "\n"
            s += str(i) + ":" + str(self.board[0][i]) + "-" + str(self.board[1][i]) + " "
        s += str(castles_owned[0]) + "-" + str(castles_owned[1])
        return s
...
class BlottoSwarmGrader:
    ...
    def grade(self, strategy1, strategy2, seed = 0):
        scores = [0, 0]
        for game_num in range(1, self.num_games+1):
            # random.seed(seed if seed else (game_num - 1) // 2)
            if game_num % 2 == 1:
    ...
    def grade_spsa(self, strategy1, strategy2):
        scores = [0, 0]
        for game_num in range(1, self.num_games+1):
            if game_num % 2 == 0:
                print(f"Progress: {game_num} / {self.num_games} | {scores}")
